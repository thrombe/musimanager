...
class manager:
    ...
    def temp_trigger(self):
        import traceback
        dict = newpipe_db_handler().extract_from_zip()
        everything(
            dict, 
            self.np_playlists, 
            self.musi_path, 
            self.musisorter_path, 
            self.musicache_path,
            )
        input("\n\npress enter to exit")
...
# downloads, caches the song info, tags music
...
# tags music by getting cached info
...
# checks if music is in correct folders and sorts them
...
# manages~~ the sorter and cache files, calls bongo and sort
